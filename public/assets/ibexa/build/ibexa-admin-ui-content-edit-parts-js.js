...
(function (global, doc, ibexa, Translator, moment) {
  ...
  var fitSections = function fitSections() {
    var contentColumn = doc.querySelector('.ibexa-main-container__content-column');
    var lastSection = doc.querySelector('.ibexa-anchor-navigation-sections .ibexa-anchor-navigation-sections__section:last-child');

    if (lastSection && lastSection.offsetHeight) {
      var lastSectionHeight = lastSection.offsetHeight;
      var headerHeight = doc.querySelector('.ibexa-edit-header').offsetHeight;
      var contentColumnBodyHeight = contentColumn.offsetHeight - headerHeight;
      var heightDiff = contentColumnBodyHeight - lastSectionHeight;

      if (heightDiff > 0) {
        lastSection.style.paddingBottom = "".concat(heightDiff, "px");
      }
    }
  };
...
(function (global, doc, ibexa, React, ReactDOM, Translator) {
  ...
  [].concat(_toConsumableArray(doc.querySelectorAll(SELECTOR_FIELD_MULTIPLE)), _toConsumableArray(doc.querySelectorAll(SELECTOR_FIELD_SINGLE))).forEach(function (fieldContainer) {
    ...
    var openUDW = function openUDW(event) {
      ...
      ReactDOM.render(React.createElement(ibexa.modules.UniversalDiscovery, _objectSpread(_objectSpread({
        onConfirm: onConfirm,
        onCancel: closeUDW,
        title: title,
...
(function (global, doc) {
  ...
  var toggleExtraActionsWidget = function toggleExtraActionsWidget(dataset) {
    var actions = doc.querySelector(".ibexa-extra-actions[data-actions=\"".concat(dataset.actions, "\"]"));
    ...
    var focusElement = actions.querySelector(dataset.focusElement);
